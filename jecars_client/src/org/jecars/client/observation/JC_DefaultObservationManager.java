          ...
          for (eventListener list : mListeners) {
            ...
            try {
              ...
              if (node.getUpdateAsHead()) {
                // **** The node is updated
                if (eventTypes.contains( JC_Event.TYPE.NODE_CHANGED )) {
                  events.add( new JC_DefaultEvent( Calendar.getInstance(), String.valueOf(EVENTID++), node.getPath_JC(), JC_Event.TYPE.NODE_CHANGED ) );
                }
              }
