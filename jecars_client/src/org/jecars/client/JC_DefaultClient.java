...
/*    
    ...
    try {
      HttpURLConnection conn = comm.createHttpConnection( getServerPath() + pPath ); 
      JD_Taglist   tags = comm.sendMessageGET( conn ); 
      if (JC_RESTComm.getResponseCode(tags)==HttpURLConnection.HTTP_OK) {
        s = new JC_DefaultStream();
        s.setStream( JC_RESTComm.getResultStream( tags ) );
        s.setContentType( (String)tags.getData( "ContentType" ) );
...
*/
  ...
  public JC_Nodeable getRootNode() throws JC_Exception {
    if (mRootNode==null) {
      try {
        mRootNode = (JC_RootNode)createNodeClass( "[root]" );
        mRootNode.setClient( this );
        mRootNode.setPath( "/" );
      } catch( Exception e ) {
