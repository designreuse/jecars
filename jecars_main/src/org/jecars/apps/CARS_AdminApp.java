...
public class CARS_AdminApp extends CARS_DefaultInterface {
   ...
   */
  static final public void autoStartTools() throws Exception {
//  if (1==1) return;
...
//    return ef;
  ...
  public String getVersion() {
    return getClass().getName() + ": " + CARS_Main.PRODUCTNAME + " version=" + CARS_Main.VERSION_ID + " CARS_AdminApp";
  }
...
//        setParamProperty( pMain, pInterfaceNode, tool, "jecars:StateRequest", CARS_ToolInterface.STATEREQUEST_START );
   ...
   */
  private void jecars_GC( CARS_Main pMain ) throws RepositoryException, ItemStateException, IOException {
    System.gc();
    return;
  }
  ...
  protected void adminApp_AccessManager_clear( final CARS_Main pMain, final Node pInterfaceNode, final Node pParentNode ) throws Exception {
    CARS_AccessManager.gClearCache();
  ...
  protected void jecars_Config( final CARS_Main pMain ) {
    ...
    ac.setErrorCode( HttpURLConnection.HTTP_OK );
    String report;
    report  = "JeCARS Configuration\n================================\n\n";
    report  += "JeCARS version\t=\t" + CARS_Main.VERSION + '\n';
    report  += "Starttime\t=\t" + CARS_Factory.getJeCARSStartTime().getTime().toString();
...
//    report += CARS_ObservationServer.startObservation( pOBS ) + '\n';
  ...
  protected void adminApp_AccessManager_status( final CARS_Main pMain ) {
    ...
    report += "Cache size = " + CARS_AccessManager.getCacheSize() + " bytes\n";
    
    report += "\nREAD PATH CACHE\n==================================\n\n";
    HashSet<String> cache = CARS_AccessManager.getReadPathCache();
    for (String c : cache) {
      report += c + '\n';
    }

    report += "\nWRITE PATH CACHE\n==================================\n\n";
    cache = CARS_AccessManager.getWritePathCache();
    for (String c : cache) {
      report += c + '\n';
    }

    report += "\nDENY READ PATH CACHE\n==================================\n\n";
    cache = CARS_AccessManager.getDenyReadPathCache();
    for (String c : cache) {
      report += c + '\n';
    }
