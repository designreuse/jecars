...
import org.jecars.output.CARS_OutputGenerator_Atom;
import org.jecars.output.CARS_OutputGenerator_Backup;
import org.jecars.output.CARS_OutputGenerator_HTML;
...
//        String nms[] = CARS_Factory.gSystemCarsSession.getNamespacePrefixes();
//        for( int i=0; i<nms.length; i++ ) {
//          if (gIncludeNS.contains( nms[i] )) {
//            gFeedHeader += " xmlns:" + nms[i] + "=\"" + CARS_Factory.gSystemCarsSession.getNamespaceURI( nms[i] ) + "\"";
//          }
//        }
        gFeedHeader += ">\n";
      }
  ...
  static public CARS_ActionContext createActionContext( final CARS_ActionContext pContext ) throws CloneNotSupportedException {
    ...
    if (pContext.mParameterMap!=null) ac.mParameterMap.putAll( pContext.mParameterMap );
    ...
    if (ac.mQueryParametersTL!=null) ac.mQueryParametersTL = (JD_Taglist)ac.mQueryParametersTL.clone();
    return ac;  
  }
...
//    return URLEncoder.encode( pS, "UTF-8" );
  ...
  private void addPropertyValue( StringBuilder pBuilder, String pPropName, Node pNode, Value pValue, String pAlt ) throws Exception {
  ...
  /** setCalanderToNowParameter
   * @param pCal
   * @param pNow
   * @return
   */
  static final private Calendar setCalanderToNowParameter( Calendar pCal, String pNow ) {
  ...
  static final private String getDateFromParam( String pDate ) {
    if (pDate.startsWith( "now" )) {
      ...
      cal = setCalanderToNowParameter( cal, pDate );
...
//    SimpleDateFormat sdf = new SimpleDateFormat( "yyyy-MM-dd'T'HH:mm:ss.SSSZ" );
    if (pDate.startsWith( "now" )) {
      Calendar cal = Calendar.getInstance();
      c = setCalanderToNowParameter( cal, pDate );
  ...
  protected RangeIterator getRangeIterator() throws Exception {
    ...
    if (mustConstructQuery( params )) {
      ...
      if ((pvalue=(String)params.getData( gDefGQL ))!=null) {
        ...
        // **** TODO
        throw new Exception( "GQL not yet implemented" );
      } else {
...
//        ni = q.execute().getNodes();
        ...
        // **** Get the rights on this object
          ...
          synchronized( sysSession ) {
            ...
            CARS_AccessManager.fillPrincipalsForGroupMembers( qm, nodes, rn.getPath() );
          
            // **** Only the permissions on the same level
            // **** "SELECT * FROM jecars:permissionable WHERE (jecars:Principal='"
            final StringBuilder qu = new StringBuilder( CARS_AccessManager.gSelectPermission );
            qu.append( rn.getPath() ).append( "'" );
            for (Node node : nodes) {
              qu.append( " OR " ).append( CARS_AccessManager.gPrincipal ).append( "='" ).append( node.getPath() ).append( "'" );
            }
            qu.append( ")" );
...
//              dpi.storeObject( n.getProperty( "jecars:Actions" ) );
        ...
        Node user = mThisNode.getSession().getRootNode().getNode( CARS_AccessManager.gUsersPath  + "/" + mUsername );
        Node temp = user.getNode( CARS_Main.DEFAULTNS + "Temp" );
        ...
        Node cntemp = temp.addNode( "childnodedefs_results_" + System.currentTimeMillis(), CARS_Main.DEFAULTNS + "datafoldermultiple" );
        ...
        for (int i=0; i<ndefs.length; i++ ) {
          ...
          for (int cni=0; cni<cnt.length; cni++ ) {
            Node n = cntemp.addNode( cnt[cni].getName(), CARS_Main.DEFAULTNS + "root" );
          }
        }
        temp.save();
        ni = cntemp.getNodes();
...
//      return "urn:uuid:" + pNode.getUUID();
   ...
   */
  private void addNodeEntry_atomsvc( StringBuilder pReply, Node pNode, String pPath ) throws RepositoryException {
    // *****************************
    // **** Atom Service
    try {
  ...
  /** Add node links
   */
  private void addNodeEntry_addlinks( boolean pSelf, StringBuilder pReply, Node pThisNode, Node pNode,
  ...
  /** Opensearch items
   */
  private void addNodeEntry_opensearch( StringBuilder pReply, Node pNode, long pFromNode, long pToNode ) {
    if (pFromNode>0) {
   ...
   * @throws java.lang.Exception
   */
  private void addNodeEntry( final StringBuilder pReply, final Node pNode, String pPath,
...
//          if (v.equals( "not" )==false) {
    ...
    if (pAlt.startsWith( "atom" )) {
      ...
      try {
        ...
        pReply.append( "<author><name>" + CARS_Main.DEFAULTAUTHOR + "</name></author>\n");
        pReply.append( "<rights>" + CARS_Main.DEFAULTRIGHTS + "</rights>\n");
//        if (pSelf==true) {
...
//        }
   ...
   */
  private void addMultiPropertyEntry( StringBuilder pReply, Node pThisNode, PropertyIterator pIt, String pAlt ) throws Exception {    
   ...
   */
  private void createCreateNodeResult( StringBuilder pReply ) {
    String alt = "atom";
    try {
   ...
   */
  private void createDeleteNodeResult( StringBuilder pReply ) {
    String alt = "atom";
    try {
      JD_Taglist params = getQueryPartsAsTaglist();
      if (params.getData( gDefAlt )!=null) alt = (String)params.getData( gDefAlt );
   ...
   * @param pReply
   */
  private void createGetNodesResult( final StringBuilder pReply ) {
    String alt = "atom";
    boolean isFeed = true;
...
//      LOG.log( Level.WARNING, null, e );
   ...
   * @param pReply
   */
  private void createError( StringBuilder pReply ) {
...
//              result = url.openStream();
              ...
              if (mThisNode.hasProperty( "jcr:mimeType" )) {
                ...
                setContentType( CARS_Mime.getMIMEType( mThisNode.getName(), null ) );
              }
            }
          } else {
            // **** Result stream set?
            if ((mResultContentsStream!=null) && (alt==null)) {
              ...
              createGetNodesResult( replyString );
              if (mError!=null) {
...
//              result = replyString;
          ...
          if ((mResultContentsStream!=null) && (alt==null)) {
            ...
            StringBuilder replyString = new StringBuilder();
            createCreateNodeResult( replyString );
            if (mError!=null) {
              replyString = new StringBuilder();
              createError( replyString );
            }
          ...
          StringBuilder replyString = new StringBuilder();
          createDeleteNodeResult( replyString );
          if (mError!=null) {
            replyString = new StringBuilder();
            createError( replyString );
          }
